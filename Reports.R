  ...
  if (!missing(account_id) & !missing(proposal_id)) {
    ...
    db_sales_report$update(query = paste0('{"time": "0"}'), 
                           update = paste0('{"$set": {"scenario-id": ', toJSON(scenario_id, auto_unbox = TRUE), 
                                           ', "hospital-sales-report-ids": ', toJSON(hospital_sales_report_ids), 
                                           ', "representative-sales-report-ids": ', toJSON(representative_sales_report_ids), 
                                           ', "product-sales-report-ids": ', toJSON(product_sales_report_ids), 
    ...
    db_personnel_assessment$update(query = paste0('{"time": "0"}'), 
                                   update = paste0('{"$set": {"representative-ability-ids": ', toJSON(rep_ability_ids), 
                                                   ', "action-kpi-ids": ', toJSON(action_kpi_ids), 
                                                   ', "paper-input-id": ', toJSON(input_id, auto_unbox = TRUE), 
